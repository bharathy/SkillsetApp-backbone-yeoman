...
function processPage(status, page, resultsKey) {
    ...
    if (status !== "success") {
        ...
        var getFailedSpecDetails = function() {
            return page.evaluate(function(){
                var SpecArray =[];
                if (SpecsQuery && SpecsQuery.length > 0) {
                    for (i = 0; i < SpecsQuery.length; ++i) {
                        var el = SpecsQuery[i];
                        if(el!=null && el.innerText) SpecArray.push(el.innerText);
                        else SpecArray.push(el);   
                    }
                } 
                return SpecArray;
            });
         ...
         var getSpecTitle = function() {
            return page.evaluate(function(){
                var title = document.body.querySelectorAll('.suite > .description')[0].innerText;
                return title;
            });
        };
        var getPassedSpecDetails = function() {
            return page.evaluate(function(){
                var SpecArray =[];
                var SpecsQuery = document.body.querySelectorAll('.passed > .description');
                if (SpecsQuery && SpecsQuery.length > 0) {
                    for (i = 0; i < SpecsQuery.length; ++i) {
                        var el = SpecsQuery[i];
                        if(el!=null && el.innerText) SpecArray.push(el.innerText);
        ...
        var ival = setInterval(function(){
            if (isFinished()) {
                ...
                var status = (failures > 0) ? "FAILURE" : "SUCCESS";
                    console.log(status + " : " + results[0]);
                    console.log("");
                    console.log(getSpecTitle());
                    console.log("");
                    if(failures > 0) {
                        console.log("FAILED SPEC :");
                        var specFailedResults = getFailedSpecDetails();
                        if (specFailedResults && specFailedResults.length > 0) {
                            for (i = 1; i < specFailedResults.length; ++i) {
                                console.log(specFailedResults[i]);
                                console.log('');
                            }
                        }
                    }
                    var specPassedResults = getPassedSpecDetails();
                    console.log("");
                    console.log("PASSED SPEC :");
                    if (specPassedResults && specPassedResults.length > 0) {
                        for (i = 1; i < specPassedResults.length; ++i) {
                            console.log(specPassedResults[i]);
                            console.log('');
                        }
                    }
